...
type resourcesServerConnector struct {
	server *grpc.Server
}
...
type resourceServer struct {
	...
	rsConnector   types.ResourceServerConnector
}

func (rsc *resourcesServerConnector) GetServer() *grpc.Server {
	return rsc.server
}

func (rsc *resourcesServerConnector) CreateServer() {
	rsc.server = grpc.NewServer([]grpc.ServerOption{}...)
}

func (rsc *resourcesServerConnector) DeleteServer() {
	rsc.server = nil
}

func (rsc *resourcesServerConnector) Listen(socketType, socketPath string) (net.Listener, error) {
	return net.Listen(socketType, socketPath)
}

func (rsc *resourcesServerConnector) Serve(socket net.Listener) {
	go func() {
		_ = rsc.server.Serve(socket)
	}()
}

func (rsc *resourcesServerConnector) Stop() {
	rsc.server.Stop()
}

func (rsc *resourcesServerConnector) Close(clientConnection *grpc.ClientConn) {
	_ = clientConnection.Close()
}

func (rsc *resourcesServerConnector) Register(client pluginapi.RegistrationClient, reqt *pluginapi.RegisterRequest) error {
	_, err := client.Register(context.Background(), reqt)
	return err
}

func (rsc *resourcesServerConnector) Dial(unixSocketPath string, timeout time.Duration) (*grpc.ClientConn, error) {
...
func newResourceServer(config *types.UserConfig, watcherMode bool, resourcePrefix, socketSuffix string) types.ResourceServer {
	...
	return &resourceServer{
		...
		rsConnector:  &resourcesServerConnector{},
	}
}
...
func (m *resourceServer) GetInfo(ctx context.Context, rqt *registerapi.InfoRequest) (*registerapi.PluginInfo, error) {
	pluginInfoResponse := &registerapi.PluginInfo{
		...
		Name:              m.resourceName,
		Endpoint:          filepath.Join(activeSockDir, m.endPoint),
